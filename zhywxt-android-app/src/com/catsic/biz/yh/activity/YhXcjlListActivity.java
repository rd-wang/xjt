...
import com.catsic.R;
import com.catsic.biz.yh.adapter.YhXcListViewAdapter;
import com.catsic.biz.yh.adapter.YhXcListViewAdapter.State;
 ...
 *
 */
public class YhXcjlListActivity extends BaseActivity{

	private @ViewInject(id=R.id.listView) ListView listView;   
    public YhXcListViewAdapter adapter;   
    /**
	 * 信息提示 ： 无数据
	 */
	...
	protected void onCreate(Bundle savedInstanceState) {
        ...
        adapter = new YhXcListViewAdapter(this, listItems); //创建适配器   
    ...
    private List<Map<String, Object>> initItems() {   
        ...
        List<YhXcjl> list =  db.findAll(YhXcjl.class,"xzqh desc");
        for(int i = 0; i < list.size(); i++) {   
            Map<String, Object> map = new HashMap<String, Object>();    
            YhXcjl obj = list.get(i);
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		switch (item.getItemId()) {
			...
			case R.id.action_add:
				startActivity(new Intent(this, YhXcjlActivity.class));
				break;
			case R.id.action_del:
				if (adapter.isChecked()) {
					//批量删除
					new YhXcjlService(this).operationDel(adapter.listItems, adapter.hasChecked);
				}else{
					//pre
