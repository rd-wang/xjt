package com.catsic.biz.lz.adapter;
...
import com.catsic.R;
import com.catsic.biz.lz.activity.LzXcjlViewActivity;
...
/**
 * @Description: 路政巡查记录列表Adapter
 * @author jxl
 * @date 2015年5月20日
 */
public class LzXcListViewAdapter extends BaseAdapter implements View.OnClickListener{
	...
	public final class ListItemView { // 自定义控件集合
		...
		public CheckBox lzxc_item_del;
	}

	public LzXcListViewAdapter(Context context,List<Map<String, Object>> listItems) {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			...
			// 获取list_item布局文件的视图
			convertView = listContainer.inflate(R.layout.activity_lz_xcjl_list_item, null);
			// 获取控件对象
			...
			listItemView.lzxc_item_del = (CheckBox) convertView.findViewById(R.id.lzxc_item_del);
		...
		if (state == State.NORMAL) {
			...
			listItemView.lzxc_item_del.setVisibility(View.INVISIBLE);
			...
			listItemView.lzxc_item_del.setVisibility(View.VISIBLE);
		}
//		
		// 设置文字
		...
		listItemView.lzxc_item_del.setChecked(false);
...
//		listItemView.btn_item_location.setOnClickListener(this);
		 ...
		 * 删除
		 */
		listItemView.lzxc_item_del.setOnCheckedChangeListener(new CheckBox.OnCheckedChangeListener() {
			@Override
	...
	public void onClick(View v) {
		...
		switch (v.getId()) {
			//查看明细
			case R.id.lz_xcjl_list_item:
				ListItemView liv =  (ListItemView) v.getTag(); 
				Intent intent = new Intent(context,LzXcjlViewActivity.class);
			...
			case R.id.btn_item_oper:
				...
				try {
					new LzXcjlService(context).operationSB(listItems, selectID);
	...
	private class ContextMenuListener implements OnCreateContextMenuListener{
		...
		public void onCreateContextMenu(ContextMenu menu, View v,ContextMenuInfo menuInfo) {
			...
			LzXcjl obj = db.findById(liv.crowid, LzXcjl.class);
