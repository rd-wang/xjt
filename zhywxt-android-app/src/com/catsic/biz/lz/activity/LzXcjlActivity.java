 ...
 */
public class LzXcjlActivity extends BaseActivity implements OnTouchListener, android.view.View.OnClickListener {

    private
    @ViewInject(id = R.id.lz_lxmc)
    EditText lz_lxmc;
    public
    @ViewInject(id = R.id.lz_kssj)
    EditText lz_kssj;
    private
    @ViewInject(id = R.id.lz_jssj)
    EditText lz_jssj;
    private
    @ViewInject(id = R.id.lz_fzr)
    EditText lz_fzr;
    private
    @ViewInject(id = R.id.lz_jlr)
    EditText lz_jlr;
    private
    @ViewInject(id = R.id.lz_xcry)
    EditText lz_xcry;
    private
    @ViewInject(id = R.id.lz_weather)
    EditText lz_weather;
    private
    @ViewInject(id = R.id.lz_fxwt)
    EditText lz_fxwt;
    private
    @ViewInject(id = R.id.lz_clyj)
    EditText lz_clyj;
    private
    @ViewInject(id = R.id.lz_cljg)
    EditText lz_cljg;
    BootstrapButton btn_save;
    private
    @ViewInject(id = R.id.btn_xzlx)
    ImageView btn_xzlx;
    private
    @ViewInject(id = R.id.gv_images)
    GridView gv_images;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_lz_xcjl_add);
        ...
        lz_kssj.setOnTouchListener(this);
        lz_jssj.setOnTouchListener(this);
        ...
        gv_images.setOnItemClickListener(new OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> arg0, View view, int position, long arg3) {
                if (position == Bimp.bmp.size()) {//添加(点击最后一张图片 +号)
                    ...
                    new ImagePopupWindows(LzXcjlActivity.this, PhotoAlbumActivity.class, gv_images, imageFileName);
                } else {//查看
                    Intent intent = new Intent(LzXcjlActivity.this, PhotoActivity.class);
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.btn_save:
                if (save()) {
                    startActivity(new Intent(this, LzXcjlListActivity.class));
                }
                break;
            case R.id.btn_xzlx:
    ...
    private boolean save() {
        ...
        LzXcjl xcjl = new LzXcjl();
        xcjl.setCrowid(UUID.randomUUID().toString());
        xcjl.setLxbm(lxbm);
        xcjl.setLxmc(lz_lxmc.getText().toString().trim());
        xcjl.setXzqh(lxjgxzqh);
        xcjl.setXcsjStart(lz_kssj.getText().toString().trim());
        xcjl.setXcsjEnd(lz_jssj.getText().toString().trim());
        xcjl.setWeather(lz_weather.getText().toString().trim());
        xcjl.setFzr(lz_fzr.getText().toString().trim());
        xcjl.setJlr(lz_jlr.getText().toString().trim());
        xcjl.setXcry(lz_xcry.getText().toString().trim());
        xcjl.setFxwt(lz_fxwt.getText().toString().trim());
        xcjl.setClyj(lz_clyj.getText().toString().trim());
        xcjl.setCljg(lz_cljg.getText().toString().trim());
    ...
    public boolean onTouch(View v, MotionEvent event) {
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                if (v.getId() == R.id.lz_kssj) {
                    ProgressDialogUtil.show(this, "正在加载...", true);
                    DateTimePickerUtil.showDateTimePicker(this, lz_kssj);
                    ProgressDialogUtil.dismiss();
                } else if (v.getId() == R.id.lz_jssj) {
                    ProgressDialogUtil.show(this, "正在加载...", true);
                    DateTimePickerUtil.showDateTimePicker(this, lz_jssj);
                    ProgressDialogUtil.dismiss();
                }
                break;
        }
